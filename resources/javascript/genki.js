  ...
  var Genki = {
    ...
    generateQuiz: function (o) {
      ...
      *********************************/
      ...
      if (typeof o.type === "object") {
        ...
        if (GenkiLang == "ja") {
          opts = opts.replace("Drag and Drop", "ドラッグ＆ドロップ").replace("Multiple Choice", "選択式").replace("Writing Practice", "書き練習").replace("Spelling Practice", "スペルの練習").replace("Write the Definition", "言葉を書く").replace("Written", "筆記テスト").replace("Stroke Order Practice", "書き順の練習").replace("Extended Drawing Practice", "連続の書き順の練習").replace("Drawing Practice", "連続の書き順の練習").replace("Fill in the Chart", "チャートを書き込む").replace("Write the Numbers", "数を書く")
        }
        ...
        modal = GenkiModal.open({
          ...
          content:
            '<span class="en">Please select the type of exercise you would like to do, then click "Begin" to start studying.</span><span class="ja">練習型を選択してから練習を始めるために「始める」をクリックしてください。</span><br><br>' +
            '<div class="center">' +
            (/\/vocabulary-index\/|\/custom-vocab\//.test(window.location) ? "" : "<div>" + '<b><span class="en">Current Exercise</span><span class="ja">今の練習</span></b><br>' + document.title.replace(/ \| Genki Study Resources.*$/, "") + "</div><br>") +
            "<div>" +
            ...
            "</div><br>" +
            '<div title="' +
            (GenkiLang == "ja" ? "練習型を変更したいなら練習ページの下の「練習型を変更する」をクリックしてください。" : "The exercise type can still be changed via the Change Exercise Type button at the bottom of an exercise.") +
            '">' +
            ...
            if (/vocab|kana|numbers|kanji/.test(o.format)) {
              ...
              if (o.format == "kana" && o.type != "kana") {
                ...
                o.info = o.info.replace(
                  /%\{KANA\}/g,
                  Genki.active.exercise[1].replace(/.*?(Hiragana|Katakana).*/, function ($1) {
              ...
              if (o.format == "kanji") {
                ...
                if (o.readings) {
                  ...
                  for (i in o.quizlet) {
                    ...
                    o.quizlet[i] = (yomikata[0] ? '<span class="yomikata" title="' + (GenkiLang == "ja" ? "音読み" : "on-yomi") + '">▶</span>' + yomikata[0] + "<br>" : "") + (yomikata[1] ? '<span class="yomikata" title="' + (GenkiLang == "ja" ? "訓読み" : "kun-yomi") + '">▷</span>' + yomikata[1] : "")
                  }
              ...
              if (o.type == "multi") {
                ...
                for (i = 0, j = keys.length; i < j; i++) {
                  ...
                  quizlet.push({
                    question: '<div class="multi-vocab">' + (def[1] ? "<ruby>" : o.format == "kanji" ? '<div class="big-kanji">' : "") + def[0] + (def[1] ? "<rt>" + def[1] + "</rt></ruby>" : o.format == "kanji" ? "</div>" : "") + (sentence ? '<hr><div class="multi-vocab-sentence">' + sentence + "</div>" : "") + "</div>",
      ...
      if (o.type == "drag") {
        ...
        while (keysQ.length) {
          ...
          quiz += '<div class="quiz-item-group">' + '<div class="quiz-item" ' + (helper || "") + ">" + (helper ? keysQ[i].replace(/(.*?)\|(.*)/, '$1<span class="hidden-text">$2</span>') : keysQ[i]) + "</div>" + "</div>"
      ...
      // # 2. KANA DRAG AND DROP #
        ...
        for (; i > -1; i--) {
          ...
          for (k in kana[i]) {
            quiz += '<div class="quiz-item-row">' + '<div tabindex="0" class="quiz-answer-zone" data-text="' + kana[i][k] + '" data-mistakes="0"></div>' + '<div class="quiz-item">' + kana[i][k] + "</div>" + "</div>"
      ...
      // # 4. MULTIPLE CHOICE #
        ...
        for (; i < j; i++) {
          quiz +=
            ...
            (o.questionsAlignLeft ? '<div class="quiz-question-inner-text">' : "") +
            (typeof q[i].question != "undefined"
              ...
              : '<div class="text-passage' + (q[i].vertical ? " vertical-text" : "") + '" ' + (q[i].text.replace(/<br>/g, "").length < 50 ? 'style="text-align:center;"' : "") + ">" + q[i].text + "</div>" + (q[i].helper || "")) +
            (o.questionsAlignLeft ? "</div>" : "") +
            "</div>" +
            (vocab ? '<button class="button vocab-spoiler-toggle" onclick="Genki.toggle.vocabSpoiler(this);"><i class="fa"></i><span class="en">Show Choices</span><span class="ja">選択肢を表示する</span></button><div class="vocab-spoiler">' : "")
      ...
      // # 5. FILL IN THE BLANKS #
        ...
        zone.innerHTML =
          '<div id="quiz-info">' +
          o.info +
          '<br><span class="en">If you don\'t know how to type in Japanese on your computer, please visit our help page by <a href="/help/writing/' +
          Genki.local +
          '" target="_blank">clicking here</a>.</span><span class="ja">パソコンで日本語を入力する方法がわからない場合は、<a href="/help/writing/' +
          Genki.local +
          '" target="_blank">ヘルプページ</a>をご覧ください。</span></div><div class="text-block">' +
          o.quizlet.replace(/\{.*?\}/g, function (match) {
            ...
            if (data[0] == "!IMG") {
              ...
              ;(hint = data[1] ? data[1] : ""), (flag = (data[10] || data[9] || data[8] || data[7] || data[6] || data[5] || data[4] || data[3] || data[2] ? data[10] || data[9] || data[8] || data[7] || data[6] || data[5] || data[4] || data[3] || data[2] : "").split(";"))
                  ...
                  : flag[1] && /width:/.test(flag[1])
                    ...
                    : flag[2] && /width:/.test(flag[2])
                      ...
                      : // automatic width calculation between alternate answer params; %(/..)
                        sub
                        ? (hint ? ((sub[0] && sub[0][1]) || "/").split("/").concat(((sub[1] && sub[1][1]) || "/").split("/")) : ((sub[0] && sub[0][1]) || "/").split("/")).sort(function (a, b) {
                            return b.length - a.length
                          })[0].length *
                            (14 / (/[a-z]/i.test(hint || data[0]) && !/[\u3000-\u30FF]/.test(hint || data[0]) ? 2 : 1)) +
                          14 +
              ...
              placeholder = /placeholder:/.test(flag[0]) ? flag[0].split(":")[1] : flag[1] && /placeholder:/.test(flag[1]) ? flag[1].split(":")[1] : flag[2] && /placeholder:/.test(flag[2]) ? flag[2].split(":")[1] : null
              ...
              return (
                ...
                '" ' +
                'style="width:' +
                width * (storageOK && localStorage.genkiFontSize ? +localStorage.genkiFontSize / 100 : 1) +
                'px;"' +
                ">" +
                ((hint && !/answer|furigana|placeholder/.test(flag[0])) || (flag[1] && /hint:/.test(flag[1])) || (flag[2] && /hint:/.test(flag[2])) ? '<span class="problem-hint">' + (flag[1] && /hint:/.test(flag[1]) ? flag[1].split(":")[1] : flag[2] && /hint:/.test(flag[2]) ? flag[2].split(":")[1] : hint) + "</span>" : "") +
                "</span>"
              )
            }
          }) +
          "</div>" +
          Genki.lang.check_answers.replace("()", "(false, 'fill')").replace("</div>", helper ? Genki.lang.toggle_furigana + "</div>" : "</div>")
      ...
      // # 6. STROKE ORDER #
        ...
        for (; i < j; i++) {
          ...
          quiz +=
            ...
            (q[i].kana ? getPaths() + "resources/images/stroke-order/sasagami-" + o.kanaType.toLowerCase() + ".jpg" : "https://jisho.org/search/" + q[i].kanji + "%20%23kanji") +
            '" target="_blank" title="' +
            (GenkiLang == "ja" ? "jisho.orgで書き順を見る" : "View stroke order on jisho.org") +
            '"><button class="button"><i class="fa">&#xf002;</i></button></a>' +
            '<a href="' +
            img +
            '" target="_blank" title="' +
            (GenkiLang == "ja" ? "クリックして画像を見る" : "Click to view image") +
            '"><img src="' +
            img +
            '" alt="' +
            (GenkiLang == "ja" ? "書き順" : "stroke order") +
            '"/></a>' +
            "</div>" +
            "</div>" +
            ...
            '"' +
            (q[i].kana && !q[i].font ? ' data-font="NotoSansJP, SawarabiGothic, MS Gothic, Yu Gothic, Meiryo"' : q[i].font ? ' data-font="' + q[i].font + '"' : "") +
            ' id="canvas-' +
            i +
            ...
            i +
            '" title="(CTRL+X)"><i class="fa">&#xf12d;</i><span class="en">Erase</span><span class="ja">消す</span></button>' +
            '<button class="button" onclick="KanjiCanvas.deleteLast(this.dataset.canvas)" data-canvas="canvas-' +
            i +
            '" title="(CTRL+Z)"><i class="fa">&#xf0e2;</i><span class="en">Undo</span><span class="ja">取り消す</span></button>' +
            ...
            '</span><span class="ja">次の' +
            (q[i].kana ? (GenkiLang == "ja" ? (/hiragana/i.test(o.kanaType) ? "ひらがな" : "カタカナ") : o.kanaType) : "漢字") +
            "</span></div>" +
            "</div>" +
            // end question block
            "</div>"
        ...
        zone.innerHTML =
          ...
          '<div id="review-exercise" class="center clearfix">' +
          '<button id="toggle-stroke-order" class="button" onclick="Genki.toggle.strokeOrder(this);"><i class="fa">&#xf1fc;</i><span class="en">' +
          (strokeOrderHidden ? "Show" : "Hide") +
          ' Stroke Order</span><span class="ja">書き順を' +
          (strokeOrderHidden ? "表示" : "非表示") +
          "する</span></button>" +
          '<button id="toggle-tracing-guide" class="button" onclick="Genki.toggle.tracingGuide(this);"><i class="fa">&#xf031;</i><span class="en">' +
          (guideHidden ? "Show" : "Hide") +
          ' Tracing Guide</span><span class="ja">トレースのガイドを' +
          (guideHidden ? "表示" : "非表示") +
          "する</span></button>" +
      ...
      // # 7. DRAWING PRACTICE #
        ...
        for (; i < j; i++) {
          ...
          // create a new row
          quiz += '<div class="quiz-answer-row">' + '<div class="drawing-zone" ' + width + ">" + '<div class="quiz-item">' + '<div class="quiz-item-text' + (q[i].kana ? " kana-font" : "") + '"' + (q[i].font ? ' style="font-family:' + q[i].font + '"' : "") + ">" + q[i].kanji + "</div>" + "</div>" + '<button class="button stroke-order-button" onclick="Genki.viewStrokeOrder(\'' + q[i].kanji + "', '" + q[i].order + "'" + (q[i].kana ? ",'" + o.kanaType + "'" : "") + ');"><span class="en">Stroke Order</span><span class="ja">書き順を見る</span></button>' + "</div>"
    ...
    endQuiz: function (type) {
      ...
      document.getElementById("quiz-result").innerHTML =
        ...
        '<div id="result-list">' +
        '<div class="result-row"><span class="result-label"><span class="en">Problems Solved:</span><span class="ja">問題を解いた：</span></span>' +
        problems +
        "</div>" +
        '<div class="result-row"><span class="result-label"><span class="en">Answers Wrong:</span><span class="ja">不正解の問題：</span></span>' +
        Genki.stats.mistakes +
        "</div>" +
        '<div class="result-row"><span class="result-label"><span class="en">Score:</span><span class="ja">得点：</span></span>' +
        Genki.stats.score +
        "%</div>" +
        '<div class="result-row"><span class="result-label"><span class="en">Completion Time:</span><span class="ja">かかった時間：</span></span>' +
        timer.innerHTML +
        "</div>" +
        '<div class="result-row center">' + // depending on the score, a specific message will show
        (Genki.stats.score == 100 ? '<span class="en">PERFECT! Great Job, you have mastered this quiz! Feel free to move on or challenge yourself by trying to beat your completion time.</span><span class="ja">満点！たいへんよくできました！</span>' : Genki.stats.score > 70 ? '<span class="en">Nice work!</span><span class="ja">よくできました！</span> ' + Genki.lang[type ? type + "_mistakes" : "mistakes"] : '<span class="en">Keep studying!</span><span class="ja">頑張りましょう！</span> ' + Genki.lang[type ? type + "_mistakes" : "mistakes"]) +
        (document.querySelector(".alt-phrase") ? "<br><br>" + Genki.lang.sub_answers : "") +
        '<div class="center">' +
        (/\/dictionary\//.test(window.location) ? Genki.lang.back_to_dict : '<button class="button" onclick="Genki.reset();"><i class="fa">&#xf021;</i><span class="en">Try Again</span><span class="ja">やり直す</span></button>') +
        ...
        '" class="button"><i class="fa">&#xf015;</i><span class="en">Back to Index</span><span class="ja">トップページに戻る</span></a>' +
        "</div>" +
        "</div>" +
        "</div>"
          ...
          if (++Genki.dataBackupReminderCount >= 10) {
            ...
            setTimeout(function () {
              GenkiModal.open({
                ...
                content:
                  '<span class="en">You\'ve recently completed 10 exercises. Would you like to backup your exercise score data?</span><span class="ja">練習が10回終わりました。練習の得点データを保存しますか？</span><br><br>' +
                  '<div class="center">' +
                  '<a id="save-exercise-data" class="button" download="Genki Exercise Score Data.txt" href="data:text/plain;charset=utf-8,' +
                  (storageOK && localStorage.Results ? encodeURIComponent(localStorage.Results.replace(/\n/g, "\r\n")) : "") +
                  '"><i class="fa">&#xf019;</i><span class="en">Save Data</span><span class="ja">セーブする</span></a><br><br>' +
                  '<div title="' +
                  (GenkiLang == "ja" ? "このポップアップを無効にします。\nデータを保存するリマインダーが再有効にしたいなら設定で有効にできます。" : "Stops this popup from showing every 10 exercises.\nYou can re-enable the data backup reminder via the settings manager. ") +
                  '">' +
                  ...
                  '<div class="donate-text">If Genki Study Resources has helped you with your studies, please also consider <a href="' +
                  getPaths() +
                  "donate/" +
                  Genki.local +
                  '" target="_blank">making a donation <i class="fa">&#xf08e;</i></a> if you can, to help support the continued development and maintenance of these resources. Your support is greatly appreciated!</div>' +
                  "</div>" +
                  "</div>" +
                  "</div>",
                zIndex: "low",
    ...
    breakTime: function () {
      GenkiModal.open({
        ...
        content:
          '<span class="en">Taking a break and waiting before trying again can greatly help with building your memory. 5 to 10 minute breaks are recommended, but you\'re free to adjust the time to your liking.' +
          (/"format":"vocab"/.test(Genki.exerciseData) ? ' Please see <a href="' + getPaths() + "help/vocab-memorization/" + Genki.local + '" target="_blank">this page</a> for more tips on memorizing vocab.' : "") +
          '</span><span class="ja">休憩してからもう一度やってみると記憶力のために役立ちます。５、１０分の休憩がおすすめですが、お好みの時間が調整できます。' +
          (/"format":"vocab"/.test(Genki.exerciseData) ? '単語を暗記するためのヒントには<a href="' + getPaths() + "help/vocab-memorization/" + Genki.local + '" target="_blank">このページ</a>をご覧ください。' : "") +
          "</span><br><br>" +
          '<div class="center"><span class="en">Wait </span><span class="ja">待つ時間：</span><input id="break-minutes" class="center" type="number" value="' +
          Genki.breakTimer[Genki.breakMultiplier] +
          '" min="1" max="60" onchange="Genki.changeBreakMultiplier(this);"> <span class="en">Minute(s)</span><span class="ja">分</span></div>',
      ...
      value: function (input) {
        ...
        // checks if currently busy processing the previous answer
          ...
          window.setTimeout(function () {
            ...
            if (document.activeElement && document.activeElement.value && document.activeElement.value == Genki.input.map[Genki.input.index - 1].value) {
                  ...
                  // increment mistakes if the answer is incorrect
                    ...
                    if (type == "fill") {
                      input[i].parentNode.insertAdjacentHTML(
                        "beforeend",
                        ('<span class="problem-answer">' + data.answer + (data.answer2 || data.furigana ? '<span class="secondary-answer' + (data.furigana ? " furigana-only" : "") + '">' + (data.answer2 || data.furigana) + "</span>" : "") + "</span>").replace(/%\((.*?)\)/g, function (Match, $1) {
    ...
    create: {
      ...
      exerciseList: function () {
        var main =
          ...
          "help/" +
          Genki.local +
          '"><i class="fa">&#xf059;</i><span class="en">Help</span><span class="ja">FAQ</span></a></div>' +
        ...
        if (Genki.exercises) {
          var attrs = 'class="lesson-title" onclick="Genki.toggle.list(this);" onkeydown="event.key == \'Enter\' && Genki.toggle.list(this);" tabindex="0"', // lesson-title attrs
            list = '<nav id="exercise-list">' + '<h3 class="main-title"><span class="en">Exercise List</span><span class="ja">練習問題一覧</span></h3>' + '<button id="random-exercise" class="button" onclick="Genki.randomExercise();" title="' + (GenkiLang == "ja" ? "ランダム練習" : "Random Exercise") + '"><i class="fa">&#xf074;</i></button>' + '<div id="lessons-list"><h4 ' + attrs + '><span class="en">Page Links</span><span class="ja">ページリンク</span></h4><ul id="page-links">' + main + "</ul>",
            lesson = "\\.\\.\\/",
            i = 0,
          ...
          for (; i < j; i++) {
            ...
            if (!new RegExp(lesson).test(linkData[0])) {
              lesson = /^appendix/.test(linkData[0]) ? "appendix" : /^study-tools/.test(linkData[0]) ? "study-tools" : linkData[0].replace(/(lesson-\d+)\/.*/, "$1")
        ...
        // creates quick nav for non-exercise pages
          ...
          var nav = '<a href="#toggle-navigation" id="toggle-exercises" onclick="Genki.toggle.exerciseList(this); return false;" title="' + (GenkiLang == "ja" ? "クイックナビゲーションをトグルする" : "Toggle quick navigation") + '"></a>' + '<nav id="exercise-list">' + '<h3 class="main-title"><span class="en">Quick Navigation</span><span class="ja">クイックナビゲーション</span></h3>' + '<div id="lessons-list">' + main + "</div>" + "</nav>"
    ...
    quickJisho: {
      ...
      create: function () {
        ...
        box.innerHTML =
          ...
          '<i class="fa fa-hover" tabindex="0" onclick="Genki.quickJisho.toggle();" onkeydown="event.key == \'Enter\' && Genki.quickJisho.toggle();" title="' +
          (GenkiLang == "ja" ? "最小化" : "Minimize") +
          '">&#xf2d1;</i>  ' +
          "</div>" +
          '<div id="quick-jisho-content">' +
          '<div class="quick-jisho-row center">' +
          '<input tabindex="0" id="quick-jisho-search" type="text" placeholder="' +
          (GenkiLang == "ja" ? "探す…" : "Search...") +
          '" oninput="Genki.quickJisho.search(this.value);">' +
          "</div>" +
              ...
              for (i = 0, j = Genki.jisho[k].length; i < j; i++) {
                for (l in Genki.jisho[k][i]) {
                  if (Genki.jisho[k][i][l].toLowerCase().indexOf(value) != -1) {
                    ...
                    results += '<li tabindex="0" class="definition clear">' + '<span class="def-ja' + (ja[1] ? " def-furi" : "") + '">' + ja[0] + (ja[1] ? "<i>" + ja[1] + "</i>" : "") + "</span>" + '<span class="def-en">' + Genki.jisho[k][i].en + "</span>" + (Genki.jisho[k][i].v ? ' <span class="def-vtype">[<i>' + Genki.jisho[k][i].v + "</i>]</span>" : "") + '<span class="def-label">' + Genki.jisho[k][i].l + "</span>"
                    ;("</li>")
    ...
    viewStrokeOrder: function (kanji, order, kana) {
      ...
      GenkiModal.open({
        ...
        content:
          ...
          (kana ? " kana-font" : "") +
          '"' +
          (/り/.test(kanji) ? ' style="font-family:SawarabiGothic, MS Gothic, Yu Gothic, NotoSansJP, Meiryo;"' : "") +
          ">" +
          kanji +
          "</div>" +
          '<a class="button-link" href="' +
          (kana ? getPaths() + "resources/images/stroke-order/sasagami-" + kana + ".jpg" : "https://jisho.org/search/" + kanji + "%20%23kanji") +
          '" target="_blank" title="' +
          (GenkiLang == "ja" ? "jisho.orgで書き順を見る" : "View stroke order on jisho.org") +
          '"><button class="button"><i class="fa">&#xf002;</i></button></a>' +
          '<a href="' +
    ...
    init: function () {
      if (Genki.exercises) {
        ...
        if (Genki.active.exercise) {
          ...
          result.insertAdjacentHTML("beforebegin", '<h2 id="exercise-title" class="center" ' + (Genki.active.exercise[2] ? 'data-page="Genki ' + (lesson < 13 ? "I" : "II") + (/workbook-|wb-/.test(Genki.active.exercise[0]) ? " Workbook" : "") + ": " + Genki.active.exercise[2] + '"' : "") + ">" + (lesson == "appendix" ? "巻末" : lesson == "study-tools" ? "ツール" : "第" + lesson + "課") + " - " + Genki.active.exercise[1] + "</h2>")
        } else {
