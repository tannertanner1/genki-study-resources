...
;(function (window, document) {
        ...
        QuickSearcher.timeout = setTimeout(function () {
          ...
          if (value) {
            for (; i < QuickSearcher.exLen; i++) {
              if (
                (QuickSearcher.li[i].innerHTML
                  .replace(/||<.*?>/g, "")
                  .toLowerCase()
                  .indexOf(value.toLowerCase()) != -1 ||
                  (QuickSearcher.li[i].dataset && QuickSearcher.li[i].dataset.keywords && QuickSearcher.li[i].dataset.keywords.toLowerCase().indexOf(value.toLowerCase()) != -1)) &&
                QuickSearcher.li[i].getElementsByTagName("A")[0] &&
                !/note/.test(QuickSearcher.li[i].id)
              ) {
                ...
                clone.dataset.lesson = QuickSearcher.grammarIndex
                  ? "L" + QuickSearcher.li[i].id.replace(/l(\d+)-.*/, "$1")
                  : clone.getElementsByTagName("A")[0].href.replace(/.*?\/(lesson-\d+).*|.*?\/(study-tools).*|.*?\/(appendix).*/, function (Match, $1, $2, $3) {
                      if ($1) {
          ...
          QuickSearcher.hitsCounter.innerHTML = hits ? "(" + hits + ") " + "<a " + 'class="fa" ' + 'style="color:#F93;" ' + 'href="#copy-search-link" ' + 'title="' + (GenkiLang == "ja" ? "サーチリンクをコピーする" : "Copy the search link") + '" ' + "onclick=\"QuickSearcher.copyLink('" + value + "'); return false;\"" + ">&#xf0ea;</a>" : ""
      ...
      copyLink: function (value) {
        GenkiModal.open({
          ...
          content:
            ...
            '<textarea id="copied-search-link" onfocus="this.select();" style="width:80%;height:100px;">' +
            (window.location.protocol + "//" + window.location.host + window.location.pathname) +
            "?search=" +
            encodeURIComponent(value) +
            "#quick-search-exercises</textarea><br>" +
  ...
  if (storageOK && localStorage.Results) {
    var parsed = JSON.parse(localStorage.Results)
    var exResults = parsed["3rd"] || {},
      k
