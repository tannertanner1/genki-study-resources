# Anki Decks for Genki Study Resources

This folder contains [Anki](https://apps.ankiweb.net/) decks for Genki 3rd Edition. You can find the 3rd Edition decks in the [lessons](lessons/) folder.


## How to Download the Decks

You can download the decks from [here](https://studyresources.jp/help/anki-decks/). The instructions below are for downloading from GitHub.

To download a single deck, you can go to an edition's folder, click the deck, and then download it via the download button.

To download all decks at once, you can [download this repo](https://github.com/tannertanner1/studyresources), unzip the contents, then navigate to **resources/tools/decks** and open the folder for the 3rd Edition (lessons) to access their decks.


## How to Import the Decks

Once you have your decks, open Anki and click "Import File" (or go to File > Import...) to import your decks.

That's it, you're done. Now you're ready to study the vocab!
